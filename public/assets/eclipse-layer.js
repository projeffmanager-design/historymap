(function () {
    ...
    function ensureComparisonStyles() {
        ...
        style.textContent = `
            ...
            #${PANEL_ID} .ec-section{margin-top:8px;padding-top:8px;border-top:1px solid rgba(255,226,154,.14)}#${PANEL_ID} .ec-title{font-size:10px;color:#c8a957;text-transform:uppercase;letter-spacing:.12em;margin-bottom:5px}#${PANEL_ID} .ec-row{padding:6px 7px;margin:4px 0;border-radius:7px;background:rgba(255,255,255,.045);border-left:2px solid #f2c45b}#${PANEL_ID} .ec-record{border-left-color:#79bfff}#${PANEL_ID} .ec-meta{color:#aeb8c8;font-size:10px;margin-top:2px}#${PANEL_ID} .ec-original{color:#d7dce5;margin-top:3px;font-family:"Noto Serif KR",serif}#${PANEL_ID} .ec-note{margin-top:8px;color:#8f98a8;font-size:9px;line-height:1.4}
    ...
    function renderComparisonPanel(data, year) {
        ...
        const eventRows = events.map(event => {
            ...
            return `<div class="ec-row"><b>${dateLabel} · ${escapeHtml(event.type)}</b><div class="ec-meta">식분 ${escapeHtml(event.magnitude ?? '-')} · 사로스 ${escapeHtml(event.saros ?? '-')} · 최대식 ${escapeHtml(event.greatest?.lat ?? '-')}°, ${escapeHtml(event.greatest?.lng ?? '-')}°</div></div>`;
        }).join('');
        const statusLabel = { recorded: '관측 기록', total_recorded: '개기 기록', obscured: '기상으로 미관측', predicted_not_observed: '예측·불발' };
        const recordRows = records.map(record => `<div class="ec-row ec-record"><b>${escapeHtml(record.month || '?')}월 · ${escapeHtml(record.title)}</b><div class="ec-meta">${escapeHtml(record.source)} · ${escapeHtml(statusLabel[record.status] || record.status)}</div><div class="ec-original">${escapeHtml(record.original_content)}</div></div>`).join('');
