(function () {
  ...
  const people = v => v === null || v === undefined ? '자료 없음' : `${new Intl.NumberFormat('ko-KR', { notation: 'compact', maximumFractionDigits: 1 }).format(v)}명`;
  ...
  const statisticFields=['population_series','population_source','population_sources','modern_population','population_scale','population_overrides','population_revision','production_series','productivity_coefficient','training_coefficient','logistics_coefficient','defense_coefficient'];
  ...
  function stat(name, value, label, index, tone, extra='') { return `<div class="np-stat ${tone}"><small>${name}</small><strong>${value}</strong><span>${label}</span>${extra?`<span>${extra}</span>`:''}${gauge(index)}</div>`; }
  ...
  function details(row, country) {
    ...
    return `<div class="np-identity"><span class="np-crest">${esc(country.name.slice(0, 1))}</span><div><small>${state.tab === 'region' ? esc(country.name) + ' · 지역 보고서' : 'NATIONAL POWER · 국가 보고서'}</small><h2>${esc(row.name)}</h2><span class="np-status">모델 추정 · ${window.nationalPowerPreview ? '시연 데이터' : '기존 지도 데이터'}</span></div>${country.rank ? `<span class="np-rank">#${country.rank}<small>국가 순위</small></span>` : ''}</div>
      ...
      ${[2025,2026].includes(time().year)&&(row.population_sources?.[time().year]||state.tab==='nation'&&/대한민국|한국/.test(country.name))?`<div class="np-count-note">한국 주민등록 인구: ${time().year===2026?'2026년 8월':'2025년 12월'} 말 기준 · 외국인 제외. 국가 합계는 현재 지도에서 대한민국에 귀속된 지역의 합계입니다. <a href="https://jumin.mois.go.kr/statMonth.do" target="_blank" rel="noopener noreferrer">행정안전부 원자료 ↗</a></div>`:''}
      <div class="np-stats">${stat('⚔ 군사력', score(row.military), '동원·훈련·보급 잠재력 지수', row.military, 'military', `추정 가용 인원 ${people(row.manpower)}`)}${stat('◈ 생산력', score(row.production), '인구·생산성·거점 잠재력 지수', row.production, 'production')}${stat('♟ 추정 인구', fmt(row.population), '연결된 인구 자료 합계 · 명', row.populationScore, 'population')}</div>
      ...
      <div class="np-mobilization"><span>추정 가용 인원 <small>인구 × ${(state.rate * 100).toFixed(0)}% · 실제 병력 아님</small></span><strong>${people(row.manpower)}</strong></div>
  ...
  async function renderCountryTab(countryId, mount) {
    ...
    try {
      ...
      mount.innerHTML=`<div class="np-embedded"><div class="np-embedded-heading"><div><small>현재 지도 · ${esc(time().year)}년</small><h3>${esc(country.name)} 국력</h3></div><button type="button" data-power-analysis="${esc(id)}">전체 분석 ↗</button></div><div class="np-total"><div><span>총 국력</span><strong>${score(country.power)}<small> / 100</small></strong></div>${gauge(country.power)}<p>등록 자료 내 상대 지수 · 역사적 확정값 아님</p></div><div class="np-stats">${stat('⚔ 군사력',score(country.military),'동원·훈련·보급 잠재력',country.military,'military',`추정 가용 인원 ${people(country.manpower)}`)}${stat('◈ 생산력',score(country.production),'인구·거점 잠재력',country.production,'production')}${stat('♟ 추정 인구',fmt(country.population),'연결 지역 합계 · 명',country.populationScore,'population')}</div><section class="np-trend" aria-label="${esc(country.name)} 국력 변화"><div class="np-trend-heading"><strong>국력 변화</strong></div><div data-country-power-trend="${esc(id)}">${trendMarkup(country)}</div></section><p class="np-embedded-note">연도별 비교와 인구 변화, 역사 사건은 전체 분석에서 확인할 수 있습니다.</p><div class="np-section-title">거점·전략 자원</div><div class="np-resources"><div><span>성</span><b>${fmt(country.castleCount)}</b></div><div><span>도시·수도</span><b>${fmt(country.cityCount)}</b></div><div><span>철</span><b>${fmt(country.iron)}</b></div><div><span>말</span><b>${fmt(country.horse)}</b></div><div><span>소금</span><b>${fmt(country.salt)}</b></div></div><details class="np-evidence"><summary>산출 기준</summary><p>생산력 45% + 군사력 35% + 거점 10% + 자원 입지 10%. 지역 인구·자원 자료를 연결한 잠정 모델이며 실제 병력이나 생산량은 아닙니다.</p></details></div>`;
  ...
  function decorate() {
    ...
    chosen.forEach(el => {
      ...
      const key = c ? `${c.id}:${score(c.power)}:${score(c.military)}:${score(c.production)}:${c.population}:${c.manpower}:${c.castleCount}:${c.cityCount}` : '';
      ...
      badge.innerHTML = `<span class="np-badge-total">총 국력 <b>${score(c.power)}</b></span>${gauge(c.power)}<span class="np-badge-detail"><span>군사력 <b>${score(c.military)} · ${people(c.manpower)}</b></span><span>생산력 <b>${score(c.production)}</b></span><span>인구 <b>${fmt(c.population)}${c.population === null ? '' : '명'}</b></span></span>`;
      badge.querySelector('.np-badge-detail').insertAdjacentHTML('beforeend',`<span>성 · 도시 <b>${fmt(c.castleCount)} · ${fmt(c.cityCount)}</b></span>`);
      badge.setAttribute('aria-label', `${c.name} 총 국력 ${score(c.power)}, 군사력 ${score(c.military)}, 추정 가용 인원 ${people(c.manpower)}, 생산력 ${score(c.production)}, 인구 ${fmt(c.population)} 상세 보기`);
