...
const invalidateHeroCaches = ({ preserveRankings = false } = {}) => {
    ...
    heroRankingPageCache.clear();
    heroRankingScopeCache.clear();
...
const setHeroPublicCacheHeaders = (res, cacheState) => {
    ...
    res.set('X-Hero-Cache', cacheState);
};
...
const countTerritoryGeometryVertices = (geometry) => {
    let count = 0;
...
async function setupRoutesAndCollections() {
    ...
    if (!collections.contributions && collections.users) {
        ...
        let _lastTerritoryTileBuild = null;
        ...
        async function rebuildTerritoryTilesIncremental(reason, affectedIds = null) {
            ...
            _lastTerritoryTileBuild = {
                ...
                mode: isFullRebuild ? '전체' : '증분', reason,
                targetCount: isFullRebuild ? null : affectedIds.size,
                phase: 'starting', phaseLabel: '재빌드 준비',
                progress: 1, current: 0, total: 0, logs: []
            };
            const updateProgress = (phase, phaseLabel, progress, current = 0, total = 0, logMessage = null) => {
                if (!_lastTerritoryTileBuild || _lastTerritoryTileBuild.startedAt !== startTime) return;
                _lastTerritoryTileBuild.phase = phase;
                _lastTerritoryTileBuild.phaseLabel = phaseLabel;
                _lastTerritoryTileBuild.progress = Math.max(0, Math.min(100, Math.round(progress)));
                _lastTerritoryTileBuild.current = current;
                _lastTerritoryTileBuild.total = total;
                if (logMessage) {
                    const logs = _lastTerritoryTileBuild.logs || (_lastTerritoryTileBuild.logs = []);
                    logs.push({ at: Date.now(), message: logMessage });
            ...
            try {
                ...
                if (!isFullRebuild) {
                    ...
                    updateProgress('scan', '기존 타일에서 변경 영토 검색', 5, 0, existingFiles.length,
                        `기존 타일 ${existingFiles.length}개 검색 시작`);
                    for (let fileIndex = 0; fileIndex < existingFiles.length; fileIndex++) {
                        const filename = existingFiles[fileIndex];
                        const raw = JSON.parse(fs.readFileSync(path.join(TILES_DIR, filename), 'utf8'));
                        ...
                        if (fileIndex % 10 === 0 || fileIndex === existingFiles.length - 1) {
                            updateProgress('scan', '기존 타일에서 변경 영토 검색',
                                5 + 20 * ((fileIndex + 1) / Math.max(1, existingFiles.length)),
                                fileIndex + 1, existingFiles.length);
                        }
                    }
                    ...
                    updateProgress('scan', '영향 타일 확정', 27, affectedTileKeys.size, affectedTileKeys.size,
                        `영향받는 타일 ${affectedTileKeys.size}개 확정`);
                } else {
                    updateProgress('scan', '전체 타일 재생성 준비', 8, 0, 0, '전체 재빌드 대상으로 실행');
                }

                // 2. 영향받는 타일에 포함될 영토 조회 ──────────────────────────
                // 전체 재빌드: 모든 영토
                ...
                const territoryQuery = Object.keys(territoriesToQuery).length > 0
                    ...
                    for (const key of tileKeys) {
                        ...
                        if (!tileMap.has(key)) {
                            ...
                            tileMap.set(key, { tile_lat: lat, tile_lng: lng,
                                bounds: { north: lat+TILE_SIZE, south: lat, west: lng, east: lng+TILE_SIZE }, features: [] });
                        }
                ...
                updateProgress('write', '타일 파일 저장 준비', 72, 0,
                    isFullRebuild ? tileMap.size : affectedTileKeys.size,
                    `생성된 타일 데이터 ${tileMap.size}개`);
                ...
                if (isFullRebuild) {
                    ...
                    for (const [, tile] of tileMap) {
                        ...
                        written++;
                        updateProgress('write', '전체 타일 파일 저장', 72 + 20 * (written / Math.max(1, tileMap.size)), written, tileMap.size);
                    }
                } else {
                    // 4. 증분: 영향받는 타일 파일만 덮어쓰기 ──────────────────
                    let written = 0;
                    for (const key of affectedTileKeys) {
                        ...
                        written++;
                        updateProgress('write', '변경 타일 파일 저장', 72 + 20 * (written / Math.max(1, affectedTileKeys.size)), written, affectedTileKeys.size);
                    }
                ...
                // 5. index.json 갱신 ────────────────────────────────────────────
                updateProgress('index', '타일 인덱스 갱신', 95, 0, 0, 'index.json 갱신 시작');
        ...
        async function rebuildTerritoryTiles(reason, force = false) {
            ...
            const ids = _dirtyTerritoryIds.size > 0 ? new Set(_dirtyTerritoryIds) : null;
            await rebuildTerritoryTilesIncremental(reason, force ? null : ids);
        }
        ...
        app.get('/api/castle', async (req, res) => {
            try {
                        ...
                        try {
                            ...
                            if (updatedDocs.length > 0) {
                                ...
                                for (const doc of updatedDocs) {
                                    ...
                                    const idx = _castleCache ? _castleCache.findIndex(c => String(c._id) === idStr) : -1;
                                    const asStr = { ...doc, _id: idStr };
                                    if (_castleCache && idx >= 0) _castleCache[idx] = asStr;
                                    else if (_castleCache) _castleCache.push(asStr);
                                }
                                _castleCacheTime = Date.now();
                                _castleLastModified = Date.now();
                            }
                        } catch (e) {
                            console.warn('⚠️ updatedAt 체크 실패 (무시):', e.message);
                        }
                ...
                // 🚀 [성능 최적화] 스트리밍 커서로 대용량 castle 컬렉션 조회 (toArray() 행 방지)
                console.log(`📖 Castle 조회 시작 (필터: ${label_type || '전체'})`);
                const startTime = Date.now();
                const cursor = collections.castle.find(query);
                const castles = [];
                for await (const doc of cursor) {
        ...
        app.get('/api/heroes', async (req, res) => {
            try {
                ...
                if (cached && Date.now() - cached.at < HERO_RESULT_CACHE_TTL_MS) {
                    setHeroPublicCacheHeaders(res, 'result-hit');
                ...
                setHeroPublicCacheHeaders(res, cacheHit ? 'base-hit' : 'miss');
...
// ... 기존 DELETE 로직 유지 (kings 배열 내의 _id를 찾아 삭제)
    try {
        ...
        // GET: source_records 단건 조회 — 연결된 장소/인물에서 정확한 사료로 이동할 때 사용
        app.get('/api/source-records/:id', async (req, res) => {
            try {
                const _id = toObjectId(req.params.id);
                if (!_id) return res.status(400).json({ message: "잘못된 ID 형식입니다." });
                const record = await collections.sourceRecords.findOne({ _id });
                if (!record) return res.status(404).json({ message: "사료 기록을 찾을 수 없습니다." });
                res.set('Cache-Control', 'no-store');
                res.json(record);
            } catch (error) {
                res.status(500).json({ message: "사료 기록 조회 실패", error:error.message });
            }
        });
        ...
        app.get('/api/territories', async (req, res) => {
            try {
                ...
                for await (const t of cursor) {
                    ...
                    const props = t.properties ? { ...t.properties } : t.properties;
        ...
        app.put('/api/territories/:id', verifyAdmin, async (req, res) => {
            try {
                ...
                rebuildTerritoryTilesIncremental('영토 수정', new Set([id])).catch(e =>
                    console.error('❌ [즉시 타일 재빌드 실패]', e.message));
        ...
        // POST: 기존 simplify_territory.js를 관리자 UI에서 현재 영토에 실행
        app.post('/api/territories/:id/simplify', verifyAdmin, async (req, res) => {
            try {
                const { id } = req.params;
                const _id = toObjectId(id);
                if (!_id) return res.status(400).json({ message: '잘못된 ID 형식입니다.' });
                const targetVertices = Math.min(5000, Math.max(50, Number(req.body?.targetVertices) || 500));
                const existing = await collections.territories.findOne({ _id }, { projection: { _id: 1, geometry: 1 } });
                if (!existing?.geometry) return res.status(404).json({ message: '단순화할 영토 geometry를 찾을 수 없습니다.' });

                const beforeVertices = countTerritoryGeometryVertices(existing.geometry);
                const scriptPath = path.join(__dirname, 'simplify_territory.js');
                const output = await new Promise((resolve, reject) => {
                    require('child_process').execFile(
                        process.execPath,
                        [scriptPath, id, String(targetVertices)],
                ...
                const updated = await collections.territories.findOne({ _id }, { projection: { geometry: 1, bbox: 1 } });
                const afterVertices = countTerritoryGeometryVertices(updated?.geometry);
                territoriesCache = null;
                territoriesCacheTime = null;
                _dirtyTerritoryIds.add(id);
                _territoryDirty = true;
                rebuildTerritoryTilesIncremental('영토 단순화', new Set([id])).catch(error =>
                    console.error('❌ [단순화 타일 재빌드 실패]', error.message));
                ...
                console.error('영토 단순화 실패:', error);
                res.status(500).json({ message: '영토 단순화 실패', error: error.message });
            }
        });
        ...
        app.delete('/api/territories/:id', verifyAdmin, async (req, res) => {
            try {
                ...
                rebuildTerritoryTilesIncremental('영토 삭제', new Set([id])).catch(e =>
                    console.error('❌ [즉시 타일 재빌드 실패]', e.message));
        ...
        app.delete('/api/territories/by-osm/:osm', verifyAdmin, async (req, res) => {
            try {
                ...
                // 삭제 전 ID 수집 (증분 타일 재빌드용)
                const toDelete = await collections.territories.find(query, { projection: { _id: 1 } }).toArray();
                const deletedIds = new Set(toDelete.map(t => t._id.toString()));
                ...
                territoriesCache = null;
                territoriesCacheTime = null;
                for (const id of deletedIds) _dirtyTerritoryIds.add(id);
                _territoryDirty = true;
                console.log('🗑️ Territories 캐시 무효화됨 (DELETE by OSM)');
                if (deletedIds.size > 0) {
                    rebuildTerritoryTilesIncremental('영토 삭제(OSM)', deletedIds).catch(e =>
                        console.error('❌ [즉시 타일 재빌드 실패]', e.message));
                }
        ...
        // 랜덤 카드 다음 후보용: 상세 데이터 없이 ID 10개만 가볍게 반환한다.
        app.get('/api/hero-rankings/random-batch', async (req, res) => {
            try {
                const voterId = getHeroVoteVoterId(req);
                const limit = Math.min(10, Math.max(1, parseInt(req.query.limit, 10) || 10));
                const excludedIds = new Set(
                    String(req.query.exclude || '').split(',').map(value => value.trim()).filter(Boolean).slice(0, 30)
                );
                const candidates = await collections.kings.aggregate([
                    { $match: { 'kings.0': { $exists: true } } },
                    { $sample: { size: 16 } },
                    { $unwind: { path: '$kings', includeArrayIndex: 'kingIndex' } },
                    { $sample: { size: Math.max(40, limit * 4) } },
                    {
                        $project: {
                            country_id: 1,
                            kingIndex: 1,
                            source_ref_id: '$kings._id',
                            voted_by: '$kings.voted_by',
                            worst_voted_by: '$kings.worst_voted_by'
                        }
                    }
                ]).toArray();
                const ids = [];
                ...
                for (const row of candidates) {
                    ...
                    const heroId = kingHeroId(String(row.country_id || ''), sourceRefId);
                    if (seen.has(heroId)) continue;
                    const heroVoters = Array.isArray(row.voted_by) ? row.voted_by.map(String) : [];
                    const villainVoters = Array.isArray(row.worst_voted_by) ? row.worst_voted_by.map(String) : [];
                    if (heroVoters.includes(voterId) || villainVoters.includes(voterId)) continue;
                    seen.add(heroId);
                    ids.push(heroId);
                ...
                console.error('[GET /api/hero-rankings/random-batch] 오류:', error);
                res.status(500).json({ message: '랜덤 후보 묶음 조회 실패' });
            }
        });
        ...
        // 랭킹 첫 화면/검색/페이지용 경량 API. 선택 범위만 DB에서 읽고 현재 화면에
        // 필요한 인물의 평가글 수만 집계해 전체 공개 API의 cold-start 비용을 피한다.
        app.get('/api/hero-rankings/page', async (req, res) => {
            const requestStartedAt = performance.now();
            const timings = {};
            const finishStage = (name, startedAt) => {
                timings[name] = Math.round((performance.now() - startedAt) * 10) / 10;
            };
            try {
                const scope = ['our', 'foreign', 'all'].includes(String(req.query.scope))
                    ? String(req.query.scope)
                    : 'our';
                const page = Math.max(1, parseInt(req.query.page, 10) || 1);
                const limit = Math.min(50, Math.max(1, parseInt(req.query.limit, 10) || 15));
                const query = String(req.query.query || '').trim().toLocaleLowerCase('ko-KR');
                const sort = ['rank', 'name', 'faction', 'type', 'period', 'vote_count', 'worst_vote_count']
                    .includes(String(req.query.sort)) ? String(req.query.sort) : 'vote_count';
                const direction = String(req.query.direction) === 'asc' ? 1 : -1;
                const cacheKey = [scope, page, limit, query, sort, direction].join('|');
                const cachedPage = heroRankingPageCache.get(cacheKey);
                if (cachedPage && Date.now() - cachedPage.at < 60 * 1000) {
                    const cacheDuration = Math.round((performance.now() - requestStartedAt) * 10) / 10;
                    res.set('Server-Timing', `cache;dur=${cacheDuration}`);
                    res.set('X-Hero-Page-Cache', 'HIT');
                ...
                if (cachedScope && Date.now() - cachedScope.at < HERO_RANKINGS_CACHE_TTL_MS) {
                    ...
                    finishStage('scope_cache_ms', stageStartedAt);
                } else {
                const countryDocs = await collections.countries.find({}, {
                    projection: { _id: 1, name: 1, color: 1, ethnicity: 1 }
                }).toArray();
                finishStage('countries_ms', stageStartedAt);
                const countryMap = new Map(countryDocs.map(country => [String(country._id), country]));
                const selectedCountryIds = countryDocs
                    .filter(country => {
                        const isOur = /동이|東夷/.test(String(country.ethnicity || ''));
                        return scope === 'all' || (scope === 'our' ? isOur : !isOur);
                    })
                    .flatMap(country => [country._id, String(country._id)]);

                stageStartedAt = performance.now();
                const kingDocs = await collections.kings.find(
                    scope === 'all' ? {} : { country_id: { $in: selectedCountryIds } },
                    {
                        projection: {
                            country_id: 1,
                            'kings._id': 1,
                            ...
                            'kings.end': 1,
                            'kings.faction': 1,
                            'kings.faction_color': 1,
                            'kings.hero_type': 1,
                            'kings.type': 1,
                            'kings.source_ref_id': 1,
                            'kings.avatar_url': 1,
                            'kings.vote_count': 1,
                            'kings.worst_vote_count': 1
                        }
                    }
                ).toArray();
                finishStage('heroes_db_ms', stageStartedAt);
                ...
                kingDocs.forEach(doc => {
                    ...
                    (Array.isArray(doc.kings) ? doc.kings : []).forEach((king, index) => {
                        ...
                        const heroId = kingHeroId(countryId, sourceRefId);
                        const identityKey = normalized.person_id ? canonicalPersonHeroId(normalized.person_id) : heroId;
                        const hero = {
                            _id: heroId,
                            ...
                            death_year: normalized.death_year,
                            avatar_url: normalized.avatar_url || '',
                            vote_count: parseInt(king.vote_count || 0, 10) || 0,
                            worst_vote_count: parseInt(king.worst_vote_count || 0, 10) || 0
                        };
                        const previous = heroesByIdentity.get(identityKey);
                        if (!previous) {
                            heroesByIdentity.set(identityKey, hero);
                            return;
                        }
                        const displayHero = (hero.birth_year ?? -Infinity) >= (previous.birth_year ?? -Infinity)
                            ? hero : previous;
                ...
                heroes = [...heroesByIdentity.values()].map(hero => ({
                    ...hero,
                ...
                finishStage('scope_build_ms', stageStartedAt);
                heroRankingScopeCache.set(scope, { at: Date.now(), heroes });
                }
                stageStartedAt = performance.now();
                if (query) {
                    heroes = heroes.filter(hero => [
                        hero.name_ko, hero.name_zh, hero.title, hero.faction,
                        hero.ethnicity, hero.hero_type, hero.birth_year, hero.death_year
                    ].filter(value => value !== null && value !== undefined).join(' ')
                        .toLocaleLowerCase('ko-KR').includes(query));
                }
                const numeric = (hero, key, fallback = 0) => Number.isFinite(Number(hero[key])) ? Number(hero[key]) : fallback;
                heroes.sort((a, b) => {
                    let comparison = 0;
                    if (sort === 'rank' || sort === 'vote_count') comparison = numeric(a, 'vote_count') - numeric(b, 'vote_count');
                    else if (sort === 'worst_vote_count') comparison = numeric(a, 'worst_vote_count') - numeric(b, 'worst_vote_count');
                    else if (sort === 'period') comparison = numeric(a, 'birth_year', 999999) - numeric(b, 'birth_year', 999999);
                    else if (sort === 'name') comparison = String(a.name_ko || '').localeCompare(String(b.name_ko || ''), 'ko');
                    else if (sort === 'faction') comparison = String(a.faction || '').localeCompare(String(b.faction || ''), 'ko');
                    else if (sort === 'type') comparison = String(a.hero_type || '').localeCompare(String(b.hero_type || ''), 'ko');
                    return comparison * direction || String(a.name_ko || '').localeCompare(String(b.name_ko || ''), 'ko');
                });
                const total = heroes.length;
                const totalVotes = heroes.reduce((sum, hero) => sum + hero.vote_count, 0);
                const totalWorstVotes = heroes.reduce((sum, hero) => sum + hero.worst_vote_count, 0);
                const heroTop3 = [...heroes].sort((a, b) => b.vote_count - a.vote_count).slice(0, 3);
                const villainTop3 = [...heroes].filter(hero => hero.worst_vote_count > 0)
                    .sort((a, b) => b.worst_vote_count - a.worst_vote_count).slice(0, 3);
                const pageItems = heroes.slice((page - 1) * limit, page * limit);
                finishStage('transform_ms', stageStartedAt);

                stageStartedAt = performance.now();
                const visibleHeroes = [...new Map(
                    [...heroTop3, ...villainTop3, ...pageItems].map(hero => [hero.vote_identity, hero])
                ).values()];
                const visibleIds = visibleHeroes.map(hero => String(hero.comment_storage_id || hero._id));
                const commentRows = visibleIds.length ? await collections.heroComments.aggregate([
                    { $match: { hero_id: { $in: visibleIds } } },
                    { $group: { _id: '$hero_id', count: { $sum: 1 } } }
                ]).toArray() : [];
                const commentCounts = new Map(commentRows.map(row => [String(row._id), Number(row.count) || 0]));
                const expose = hero => {
                    const { comment_storage_id, ...publicHero } = hero;
                    return { ...publicHero, comment_count: commentCounts.get(String(comment_storage_id || hero._id)) || 0 };
                };
                finishStage('comments_ms', stageStartedAt);
                timings.total_ms = Math.round((performance.now() - requestStartedAt) * 10) / 10;
                console.log('[RankAPI/page]', { scope, page, limit, query: !!query, total, timings });
                const responsePayload = {
                    summary: { total, total_votes: totalVotes, total_worst_votes: totalWorstVotes },
                    page,
                    limit,
                    total_pages: Math.max(1, Math.ceil(total / limit)),
                    hero_top3: heroTop3.map(expose),
                    villain_top3: villainTop3.map(expose),
                    items: pageItems.map(expose),
                    timings
                };
                heroRankingPageCache.set(cacheKey, { at: Date.now(), payload: responsePayload });
                while (heroRankingPageCache.size > 100) {
                    heroRankingPageCache.delete(heroRankingPageCache.keys().next().value);
                }
                res.set('Server-Timing', Object.entries(timings).map(([name, value]) => `${name};dur=${value}`).join(', '));
                res.set('X-Hero-Page-Cache', 'MISS');
                res.set('Cache-Control', 'public, max-age=15, s-maxage=60, stale-while-revalidate=300');
                res.json(responsePayload);
            } catch (error) {
                console.error('[GET /api/hero-rankings/page] 오류:', error);
                res.status(500).json({ message: '영웅 랭킹 페이지 조회 실패' });
            }
        });
            ...
            try {
                ...
                if (!baseHeroes) {
                    const [countryDocs, kingDocs] = await Promise.all([
                        ...
                        // 공개 랭킹에는 사용자별 투표자 목록이 필요 없다. 큰 배열을 DB에서
                        // 전송하지 않아 cold cache 응답과 서버 메모리 사용량을 줄인다.
                        collections.kings.find({}, {
                            projection: {
                                country_id: 1,
                                'kings._id': 1,
                ...
                const heroes = baseHeroes.map(({ comment_storage_id, ...hero }) => ({
                    ...hero,
        ...
        app.get('/api/internal/tile-status', (req, res) => {
            ...
            res.json({
                ...
                dirtyIds: [..._dirtyTerritoryIds].slice(0, 200),
                lastBuild: _lastTerritoryTileBuild,
                retryScheduled: !!_tileRebuildRetryTimer,
                retryAttempt: _tileRebuildRetryAttempt
            });
        });
        ...
        app.get('/api/internal/deploy-status', async (req, res) => {
            ...
            res.json({
                localBuild: {
                    ...
                    dirty: _territoryDirty,
                    dirtyCount: _dirtyTerritoryIds.size,
                    // 관리 화면에서 "미반영"의 실제 대상과 직전 실패 원인을 확인할 수 있게 한다.
                    dirtyIds: [..._dirtyTerritoryIds].slice(0, 200),
                    lastBuild: _lastTerritoryTileBuild,
                    retryScheduled: !!_tileRebuildRetryTimer,
                    retryAttempt: _tileRebuildRetryAttempt
