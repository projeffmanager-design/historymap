...
app.put('/api/contributions/:id/approve', verifyToken, async (req, res) => {
    try {
        ...
        // 승인자 권한 확인 (정2품 수국사 이상) - DB position이 아닌 실시간 계산 필요
        const approverPositions = ['수국사', '판사관사', '감수국사'];
        const user = await collections.users.findOne({ _id: toObjectId(userId) });
        ...
        if (!user || !approverPositions.includes(realtimePosition)) {
            return res.status(403).json({ message: `승인 권한이 없습니다. (정2품 수국사 이상만 가능, 현재: ${realtimePosition})` });
        }
